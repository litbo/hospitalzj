package com.litbo.hospitalzj.entity;

import java.util.Date;
/**
 * 检测仪器表
 * @author 刘洋
 *
 */
public class Yq {
	private String jcyqId;      
	private String jcyqName;
	private String jcyqXh;
	private double jcyqXzzqTime;
	private String jcyqDah;       
	private String jcyqBh;
	private String jcyqCjId ;
	private Date jcyqQyTime;
	private String jcyqKsId;
	private String jcyqCf;
	private String jcyqUrl;
	private Date jcyqJzTime;
	private Integer mbId;
	
	public String getJcyqId() {
		return jcyqId;
	}
	public void setJcyqId(String jcyqId) {
		this.jcyqId = jcyqId;
	}
	public String getJcyqName() {
		return jcyqName;
	}
	public void setJcyqName(String jcyqName) {
		this.jcyqName = jcyqName;
	}
	public String getJcyqXh() {
		return jcyqXh;
	}
	public void setJcyqXh(String jcyqXh) {
		this.jcyqXh = jcyqXh;
	}
	public double getJcyqXzzqTime() {
		return jcyqXzzqTime;
	}
	public void setJcyqXzzqTime(double jcyqXzzqTime) {
		this.jcyqXzzqTime = jcyqXzzqTime;
	}
	public String getJcyqDah() {
		return jcyqDah;
	}
	public void setJcyqDah(String jcyqDah) {
		this.jcyqDah = jcyqDah;
	}
	public String getJcyqBh() {
		return jcyqBh;
	}
	public void setJcyqBh(String jcyqBh) {
		this.jcyqBh = jcyqBh;
	}
	public String getJcyqCjId() {
		return jcyqCjId;
	}
	public void setJcyqCjId(String jcyqCjId) {
		this.jcyqCjId = jcyqCjId;
	}
	public Date getJcyqQyTime() {
		return jcyqQyTime;
	}
	public void setJcyqQyTime(Date jcyqQyTime) {
		this.jcyqQyTime = jcyqQyTime;
	}
	public String getJcyqKsId() {
		return jcyqKsId;
	}
	public void setJcyqKsId(String jcyqKsId) {
		this.jcyqKsId = jcyqKsId;
	}
	public String getJcyqCf() {
		return jcyqCf;
	}
	public void setJcyqCf(String jcyqCf) {
		this.jcyqCf = jcyqCf;
	}
	public String getJcyqUrl() {
		return jcyqUrl;
	}
	public void setJcyqUrl(String jcyqUrl) {
		this.jcyqUrl = jcyqUrl;
	}
	public Date getJcyqJzTime() {
		return jcyqJzTime;
	}
	public void setJcyqJzTime(Date jcyqJzTime) {
		this.jcyqJzTime = jcyqJzTime;
	}
	public Integer getMbId() {
		return mbId;
	}
	public void setMbId(Integer mbId) {
		this.mbId = mbId;
	}
	@Override
	public String toString() {
		return "yq [jcyqId=" + jcyqId + ", jcyqName=" + jcyqName + ", jcyqXh=" + jcyqXh + ", jcyqXzzqTime="
				+ jcyqXzzqTime + ", jcyqDah=" + jcyqDah + ", jcyqBh=" + jcyqBh + ", jcyqCjId=" + jcyqCjId
				+ ", jcyqQyTime=" + jcyqQyTime + ", jcyqKsId=" + jcyqKsId + ", jcyqCf=" + jcyqCf + ", jcyqUrl="
				+ jcyqUrl + ", jcyqJzTime=" + jcyqJzTime + ", mbId=" + mbId + "]";
	}
	
}
